...
class AdvancedCharacterAssignment:
    ...
    def analizza_conflitti(self) -> Dict:
        ...
        personaggi_conflitto = {p: count for p, count in popolarita.items() if count > 1}
        personaggi_critici = {p: count for p, count in popolarita.items() 
                             if count >= n_persone * 0.6}  # >60% lo vuole
        
        # Persone a rischio (poche preferenze in zone ad alto conflitto)
        persone_rischio = []
        for persona, preferenze in self.persone_scelte.items():
            if len(preferenze) <= 2:  # Poche preferenze
                conflitti_personali = sum(1 for p in preferenze if p in personaggi_conflitto)
                if conflitti_personali >= len(preferenze) * 0.8:  # >80% delle sue preferenze sono in conflitto
        ...
        if personaggi_critici:
            suggerimenti.append(f"⚠️ Personaggi molto richiesti: {', '.join(personaggi_critici.keys())}")
            
        if persone_rischio:
            nomi = [p['persona'] for p in persone_rischio]
            suggerimenti.append(f"⚠️ Persone a rischio (poche preferenze): {', '.join(nomi)}")
            
        if personaggi_non_richiesti:
            suggerimenti.append(f"💡 Personaggi mai richiesti: {', '.join(personaggi_non_richiesti)}")
            suggerimenti.append("💡 Considera di rimuoverli o promuoverli")
            
        if len(self.tutti_personaggi) - len(self.persone_scelte) < 2:
            suggerimenti.append("⚠️ Pochi personaggi di riserva, considera di aggiungerne altri")
    ...
    def stampa_analisi_conflitti(self):
        ...
        if analisi['persone_rischio']:
            ...
            for info in analisi['persone_rischio']:
                print(f"   • {info['persona']}: {info['preferenze']} preferenze, "
    ...
    def espandi_preferenze_intelligente(self, metodo='similarità') -> Dict[str, List[str]]:
        """
    ...
    def _trova_personaggio_simile(self, persona_target: str, candidati: Set[str]) -> str:
    ...
    def assegna_con_strategia(self, strategia: str = 'hybrid', 
                            espandi_preferenze: bool = True) -> Dict[str, str]:
        """
    ...
    def _assegna_hungarian(self, preferenze: Dict[str, List[str]]) -> Dict[str, str]:
        ...
        # Matrice costi
        costi = np.full((n_persone, n_persone), 1000.0)  # Usa la stessa dimensione per righe e colonne
        ...
        return {persone[i]: personaggi[j] for i, j in zip(indici_persone, indici_personaggi)}
    ...
    def _assegna_bilanciato(self, preferenze: Dict[str, List[str]]) -> Dict[str, str]:
        ...
        # Calcola quante copie di ogni personaggio servono
        copie_necessarie = (n_persone + len(personaggi_originali) - 1) // len(personaggi_originali)
        ...
        def rarità_preferenze(persona):
            scelte = preferenze[persona]
            return sum(popolarita[p] for p in scelte) / len(scelte) if scelte else float('inf')
    ...
    def _assegna_priorita_equa(self, preferenze: Dict[str, List[str]]) -> Dict[str, str]:
    ...
    def _assegna_greedy_intelligente(self, preferenze: Dict[str, List[str]]) -> Dict[str, str]:
    ...
    def _assegna_hybrid(self, preferenze: Dict[str, List[str]]) -> Dict[str, str]:
        ...
        for strategia in strategie:
            ...
            try:
                assegnazione = getattr(self, f'_assegna_{strategia.replace("_", "_")}')(preferenze)
        ...
        print(f"🎯 Strategia ibrida: usata '{migliore[0]}' (punteggio: {migliore[2]:.2f})")
        
        return migliore[1]
    
    def _valuta_assegnazione(self, assegnazioni: Dict[str, str], 
                           preferenze: Dict[str, List[str]]) -> float:
    ...
    def confronta_strategie(self) -> Dict: 
        ...
        for strategia in self.strategie_disponibili:
            ...
            try:
                assegnazione = self.assegna_con_strategia(strategia, espandi_preferenze=False)
                ...
                if len(assegnazione) != n_persone:
                    raise ValueError(f"Assegnazioni incomplete: {len(assegnazione)}/{n_persone} persone")
                ...
                for persona, personaggio in assegnazione.items():
                    ...
                    if personaggio in scelte:
                        ...
                        preferenze_soddisfatte += 1
                        dettagli.append(f"{persona}: {personaggio} (pref #{posizione+1})")
                    else:
                        costo_totale += 1000
                ...
                print(f"   Soddisfazione: {percentuale:.1f}% ({preferenze_soddisfatte}/{len(assegnazione)})")
                print()
    ...
    def stampa_risultati_avanzati(self, assegnazioni: Dict[str, str]):
        ...
        if self.analisi_conflitti:
            persone_rischio = {p['persona'] for p in self.analisi_conflitti['persone_rischio']}
        else:
            persone_rischio = set()
        
        costo_totale = 0
        preferenze_soddisfatte = 0
        risultati_per_categoria = {'ottimi': [], 'buoni': [], 'accettabili': [], 'problematici': []}
        ...
        print(f"   • Costo totale: {costo_totale}")
        print(f"   • Preferenze soddisfatte: {preferenze_soddisfatte}/{len(assegnazioni)} ({percentuale:.1f}%)")
    ...
    def genera_report_testuale(self, assegnazioni: Dict[str, str], migliore_strategia: str) -> str:
        ...
        report.append(f"• Media preferenze per persona: {self.analisi_conflitti['media_preferenze']:.1f}\n")
        ...
        n_soddisfatti = sum(1 for p, c in assegnazioni.items() if c in self.persone_scelte[p])
        perc_soddisfazione = (n_soddisfatti / n_persone) * 100
        
        report.append(f"\nRISULTATI FINALI:")
        report.append(f"• Persone che hanno ricevuto una delle loro scelte: {n_soddisfatti}/{n_persone}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        'file_csv',
        help='Il file CSV contenente le preferenze (formato: wide o long)'
    )
    parser.add_argument(
        '--formato',
        choices=['wide', 'long'],
    ...
    # Carica dati da CSV
    try:
        assegnatore.carica_da_csv(args.file_csv, formato=args.formato, delimiter=args.delimiter)
    except Exception as e:
    ...
    assegnazione_finale = assegnatore.assegna_con_strategia(strategia_da_usare, espandi_preferenze=True)
