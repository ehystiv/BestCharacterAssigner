...
def run_evaluate(args_list: list) -> None:
    ...
    try:
        assegnatore.carica_da_csv(args_list[0], formato=args_list[1], delimiter=args_list[2])
...
def main() -> None:
    ...
    evaluate_parser = subparsers.add_parser("evaluate", help="Valuta quale sia la strategia migliore")
    evaluate_parser.add_argument("preference_file", help="File CSV con le preferenze")
    evaluate_parser.add_argument("--format", choices=["wide", "long"], default="wide",
                             help="Formato CSV (default: wide)")
    evaluate_parser.add_argument("--delimiter", default=",", help="Delimitatore CSV (default: ,)")
